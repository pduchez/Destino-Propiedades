...
export const GET = withAuth(async () => {
  // El almacenamiento ahora es la base de datos: siempre disponible si hay BD.
  return json({ falReady: falConfigured(), storageReady: true });
});
...
export const POST = withAuth(async (req) => {
  ...
  // 1) Cerebro: prompt de edición. 2) fal.ai. 3) sello. 4) guardar en BD.
  ...
  // Guardado directo en la base de datos (sin depender de Blob externo).
  const stored = await prisma.storedImage.create({
    data: { mimeType: "image/jpeg", bytes: new Uint8Array(stamped) },
  });
  ...
  return json({ asset, promptUsed: prompt, before: absSource, after: url });
});
